...
class LibRuPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        # db_authors = spider.crawler.db_authors
        # db_htmls = spider.crawler.db_htmls
        # db_titles = spider.crawler.db_titles
        # db_texts_categories = spider.crawler.db_texts_categories
        ...
        if isinstance(item, Text):  # if 'html_' in item:
            ...
            #
            #     db_titles.update({'slug': item['slug'], 'html': item['html']}, ['slug'], ensure=True)
            #     t = db_titles.find_one(slug=item['slug'])
            #
            #     for slug, name in item['categories']:
            #         if not name in self.cache_text_categories:
            #             db_texts_categories_names.insert({'slug': slug, 'name': name}, ensure=True)
            #             c = db_texts_categories_names.find_one(name=name)
            ...
            #
            #         try:
            #             db_texts_categories.insert({'cat_id': c['id'], 'text_id': t['id']}, ensure=True)
            #         except:
            #             print()

        elif isinstance(item, AuthorItem):  # elif 'name' in item:
            with spider.crawler.db_lock:
                a = db_authors.find_one(slug=item['slug'])

                if not a:
                    works = item['works']
                    del item['works']
                    db_authors.upsert(item, ['slug'], ensure=True)

                    a = db_authors.find_one(slug=item['slug'])
                    for w in works:  w.update({'author_id': a['id']})
                    db_titles.insert_many(works, ensure=True)
